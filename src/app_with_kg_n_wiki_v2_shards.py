...
load_dotenv()
# ===================== Paths / Config =====================
DEFAULT_V2_DIR = os.path.join(os.getcwd(), ".cache", "graphs_compact")
...
# ===================== Expand v2 to v1-like (Preview only) =====================
...
st.set_page_config(page_title="GitHub → Sharded KG → Wiki (QGenie + README)", layout="wide")
st.title("📚 GitHub → Sharded Knowledge Graph (Compact v2) → 📄 Wiki (QGenie, with README)")
...
with st.sidebar:
    ...
    run_btn = st.button("Build Sharded Knowledge Graph", type="primary", use_container_width=True)

tab_build, tab_preview, tab_wiki = st.tabs([
    "🔨 Build Sharded KG",
    "👀 Preview / Explore",
    "📄 Generate Wiki (QGenie)"
])
...
if run_btn:
    with tab_build:
        ...
        try:
            ...
            st.write("Scanning repository files (polyglot, compact v2)...")
            ...
            single_path = save_compact_graph_v2(v2_dir, meta, compact, gzip_out=gzip_out)
            ...
            st.code(single_path, language="bash")

            # Collect README files and save doc hints
            st.write("Collecting README files...")
            readmes = collect_readmes_text(repo_root, subpath=subpath)
            if readmes:
                hints_path = save_doc_hints(meta, v2_dir, readmes)
                st.success(f"README hints saved: {hints_path}")
            else:
                st.info("No README files detected.")
            ...
            st.info("Next: use **Preview** to explore, or **Generate Wiki** to create the documentation XML.")
...
# ===================== Preview tab (per-shard unified) =====================
with tab_preview:
    st.subheader("Explore a sharded graph (per-file view by shard)")
    try:
        ensure_dir(v2_dir)
        ...
        if not manifests:
            st.info("No manifests found. Build a sharded graph first in the Build tab.")
        else:
            selected_rel = st.selectbox("Select a manifest", options=[os.path.relpath(p, v2_dir) for p in manifests], key="preview_manifest")
            ...
            if not shards:
                st.warning("Manifest has no shards.")
            else:
                shard_labels = []
                shard_paths = []
                for s in shards:
                    ...
                    if sp and not os.path.isabs(sp):
                        sp = os.path.join(os.path.dirname(manifest_path), sp)
                    shard_paths.append(sp)
                    shard_labels.append(os.path.basename(sp))
                sel_idx = st.selectbox("Select a shard", options=list(range(len(shard_paths))),
                                       format_func=lambda i: shard_labels[i] if 0 <= i < len(shard_labels) else "shard")
                shard = load_json_autoz(shard_paths[sel_idx])
                v1 = expand_compact_to_v1(shard)
                nodes, edges = v1.get("nodes", []), v1.get("edges", [])

                nodes_by_id = {n["id"]: n for n in nodes}
                files_by_id = {n["id"]: n for n in nodes if n.get("type") == "file"}
                file_index = {
                    fid: {"path": fnode.get("path", fnode.get("label", "")), "lang": fnode.get("lang", ""),
                ...
                st.markdown("### Files (Imports, Classes, Functions) — this shard")
                ...
                sort_by = st.selectbox("Sort by", options=["path", "lang", "imports", "classes", "functions"], index=0, key="sort_by_files")
            ...
            # Show manifest JSON first lines
            with st.expander("Manifest JSON (first 200 lines)"):
                text = json.dumps(manifest, indent=2, ensure_ascii=False).splitlines()
                st.code("\n".join(text[:200]))
    except Exception as e:
        st.error(f"Failed to load cached graphs: {e}")

# ===================== Generate Wiki (QGenie, Sharded only) =====================

with tab_wiki:
    st.subheader("📄 Generate Wiki (Sharded map→reduce using QGenie)")
    try:
        ensure_dir(v2_dir)
        manifests = []
        for sub in os.listdir(v2_dir):
        ...
        if not manifests:
            st.info("No manifests found. Build a sharded graph first.")
            st.stop()

        selected_rel = st.selectbox("Select a manifest", options=[os.path.relpath(m, v2_dir) for m in manifests], key="wiki_manifest_select")
        manifest_path = os.path.join(v2_dir, selected_rel)
        manifest = load_json_autoz(manifest_path)
        ...
        readme_hints = load_doc_hints(graph_meta.get("graph_id", ""), v2_dir)
        st.markdown("### QGenie Settings")
        colA, colB = st.columns(2)
        with colA:
            qgenie_model = st.text_input("QGenie model id", value="qwen2.5-14b-1m")
            lang_text = st.text_input("Target language", value="English")
            include_readme = st.checkbox("Include README excerpt in prompts", value=True)
        with colB:
            max_files_per_shard = st.number_input("Max files per shard (prompt)", min_value=20, max_value=1000, value=200, step=10)
            readme_max_chars = st.number_input("README excerpt max chars", min_value=500, max_value=20000, value=4000, step=500)
            strict_relevance = st.checkbox("Strict relevance mode (guarded prompt + prune)", value=True)
            refine = st.checkbox("Final refine pass", value=True)

        run_sharded = st.button("Generate Wiki (Sharded → QGenie)", type="primary", use_container_width=True)

        if run_sharded:
            try:
                # README excerpt (global)
                readme_excerpt = ""
                if include_readme and readme_hints:
                ...
                for idx, s in enumerate(manifest.get("shards", []), start=1):
                    ...
                    if strict_relevance:
                        ...
                        # Non-guarded minimal prompt incl. README if opted
                        readme_block = f"\n## README context (excerpt)\n<readme>\n{readme_excerpt}\n</readme>\n" if readme_excerpt else ""
                        prompt = f"""You are a senior documentation architect.

Repository: {owner_repo or '(unknown)'}
Source: {source_url or '(unknown)'}
This prompt is for shard: {safe_shard_name}
IMPORTANT: The wiki content will be generated in {lang_text} language.
{readme_block}
Files (subset):
{json.dumps(shard_summary['files'], ensure_ascii=False, indent=2)}

{PARTIAL_XML_SPEC}
"""

                    with st.status(f"Generating partial for shard **{safe_shard_name}** via QGenie...", expanded=False):
                        xml = qgenie_generate(prompt, model=qgenie_model)
