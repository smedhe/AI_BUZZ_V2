...
"""
Page-level documentation generator (RAG + QGenie + Mermaid + Caching)

Inputs:
  - Wiki structure XML (pages + sections).
  - Hybrid embeddings bundle at .cache/embeddings/<graph_id>/:
      * units.parquet (or units.parquet.jsonl.gz)
      * file.index, file_ids.json
      * symbol.index, symbol_ids.json
  - (Optional) README used earlier is not required here.

Features:
  - Per-section retrieval boosts (architecture, CI, models, etc.).
  - Hybrid retrieval (symbols + files) with lexical boosters.
  - Two-pass generation (Draft -> Refine), Markdown-only.
  - Diagram-only fallback (sequenceDiagram etc.) if none produced.
  - "References" auto-link to GitHub blobs with owner/repo/branch/subpath.
  - Caching of QGenie outputs per-page by deterministic key.
  - Generate selected pages or all pages, with progress.

Run:
  streamlit run page_doc_generator.py
"""
...
def render_mermaid(mermaid_code: str, height: int = 500):
    """Render Mermaid (no extra pip deps)."""
    import streamlit.components.v1 as components
    safe = mermaid_code.replace("\\", "\\\\").replace("`", "\\`")
    html = f"""
    <div id="container"></div>
    https://unpkg.com/mermaid@10/dist/mermaid.min.js</script>
    <script>
    ...
    for i, line in enumerate(lines):
        ...
        if in_refs:
            if hdr.startswith("## ") and i != 0:
                ...
                out.append(line)
                continue
            # Try to detect a path token (backticked or plain)
            m_code = re.findall(r"`([^`]+)`", line)
            replaced = line
            # If backticked paths exist, linkify those
            if m_code:
                for p in m_code:
                    url = base + p.lstrip("/")
                    replaced = replaced.replace(f"`{p}`", f"`{p}`")
            else:
                # crude: find token after "- " or "* " or numeric list
                m = re.match(r"^\s*[-*]\s+(.+)$", line) or re.match(r"^\s*\d+\.\s+(.+)$", line)
                if m:
                    p = m.group(1).strip()
                    # If looks like a path (has '/' or a dot file extension), link it
    ...
    for i, pid in enumerate(page_ids, start=1):
        try:
            with st.status(f"Generating page **{pid}**...", expanded=False):
                ...
                st.markdown("---")
                st.markdown(f"### {wiki['pages'][pid].get('title')}")
                st.markdown(final_md)
                diagrams = extract_mermaid_blocks(final_md)
                if diagrams:
                    st.info(f"Rendering {len(diagrams)} Mermaid diagram(s)...")
                    for j, d in enumerate(diagrams, start=1):
